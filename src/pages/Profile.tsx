...
export default function Profile() {
  ...
  const displayName = user?.user_metadata?.display_name?.toString() || profile.username;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      >
        <div className="absolute top-0 right-0 w-64 h-64 rounded-full blur-3xl" style={{ background: 'rgba(0,255,247,0.05)' }} />
        ...
        >
          ...
          />
          <UserCircle className="w-16 h-16 text-cyan-400" style={{ filter: 'drop-shadow(0 0 8px rgba(0,255,247,0.8))' }} />
        </div>

        <div className="text-center md:text-left z-10 flex-1">
          <div className="inline-flex items-center gap-2 font-mono text-sm mb-2 px-3 py-1 rounded-full"
            style={{ color: '#00fff7', background: 'rgba(0,255,247,0.1)', border: '1px solid rgba(0,255,247,0.2)' }}>
          ...
          <div className="flex flex-wrap justify-center md:justify-start gap-6 font-mono text-sm text-muted-foreground mt-4">
            ...
            <div className="flex flex-col border-l border-border/50 pl-6">
              ...
              <span className="text-xl" style={{ color: '#bf00ff', textShadow: '0 0 10px rgba(191,0,255,0.7)' }}>{profile.rating}</span>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-8">
          <motion.div
            ...
            style={{ background: 'rgba(0,0,0,0.6)', border: '1px solid rgba(0,255,247,0.15)', backdropFilter: 'blur(16px)', borderTop: '2px solid rgba(0,255,247,0.4)' }}
          >
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={solvedData} layout="vertical" margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                  <XAxis type="number" hide />
                  <YAxis dataKey="name" type="category" axisLine={false} tickLine={false}
                    tick={{ fill: '#888', fontFamily: 'monospace', fontSize: 12 }} width={80} />
                  <Tooltip
                    cursor={{ fill: 'rgba(255,255,255,0.05)' }}
                    contentStyle={{ backgroundColor: 'rgba(0,0,0,0.8)', border: '1px solid #333', borderRadius: '4px', fontFamily: 'monospace' }}
                  />
          ...
          <motion.div
            ...
            className="p-6 rounded-xl"
            style={{ background: 'rgba(0,0,0,0.6)', border: '1px solid rgba(0,255,247,0.15)', backdropFilter: 'blur(16px)' }}
          >
            ...
            <div className="flex flex-wrap gap-2">
              {profile.recentSubmissions.map((status, i) => (
                <div
                  ...
                  style={{
                    background: status === 1 ? 'rgba(34,197,94,0.2)' : 'rgba(239,68,68,0.2)',
                    border: `1px solid ${status === 1 ? '#22c55e' : '#ef4444'}`,
                    boxShadow: status === 1 ? '0 0 5px rgba(34,197,94,0.5)' : 'none',
                  }}
        ...
        >
          ...
          <div className="w-full h-80 mt-4 relative">
            <div className="absolute inset-0 rounded-full blur-3xl" style={{ background: 'rgba(191,0,255,0.05)' }} />
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart cx="50%" cy="50%" outerRadius="70%" data={skillData}>
                <PolarGrid stroke="rgba(255,255,255,0.1)" />
                <PolarAngleAxis dataKey="subject" tick={{ fill: '#888', fontFamily: 'monospace', fontSize: 11 }} />
                <PolarRadiusAxis angle={30} domain={[0, 150]} tick={false} axisLine={false} />
                <Radar name="Skills" dataKey="A" stroke="#bf00ff" fill="#bf00ff" fillOpacity={0.4} />
              </RadarChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
