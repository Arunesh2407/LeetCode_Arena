    ...
    <div className="h-[calc(100vh-4rem)] flex flex-col md:flex-row overflow-hidden bg-background/90 backdrop-blur-xl">
      ...
      <div className="w-full md:w-1/3 lg:w-2/5 border-r border-border/50 flex flex-col overflow-y-auto">
        <div className="p-4 border-b border-border/50 flex items-center justify-between bg-black/20 sticky top-0 z-10 backdrop-blur-md">
          <Link href="/problems" className="flex items-center gap-2 text-muted-foreground hover:text-primary transition-colors font-mono text-sm">
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap mb-4">
              {problem.tags.map(tag => (
                <span key={tag} className="text-xs font-mono px-2 py-0.5 border border-primary/30 text-primary/80 bg-primary/5 rounded-sm">
                  {tag}
                </span>
              ))}
            </div>
            ...
            {problem.description.split('\n\n').map((para, i) => (
              <p key={i}>
                {para.split('`').map((text, j) =>
                  j % 2 === 1
                    ? <code key={j} className="text-cyan-400 bg-cyan-400/10 px-1 rounded font-mono">{text}</code>
                    : text
                )}
              </p>
            ))}
        ...
        <div className="h-1/3 min-h-[160px] bg-black/80 p-4 font-mono text-sm overflow-y-auto relative">
          ...
          <AnimatePresence mode="wait">
            {isRunning ? (
              ...
              >
                <span className="w-2 h-4 bg-primary animate-pulse inline-block" /> PROCESSING DATA STREAM...
              </motion.div>
            ) : output ? (
              <motion.pre
                key="output"
              ...
              <div key="idle" className="text-muted-foreground/30">Awaiting execution...</div>
            )}
          </AnimatePresence>
        </div>
      </div>
